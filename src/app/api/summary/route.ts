...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalSummary = {
      ...
      floor1RentIncome: summaries.reduce((sum, s) => sum + s.floor1RentIncome, 0),
      ...
      holidayCompensation: summaries.reduce((sum, s) => sum + s.holidayCompensation, 0),
...
async function calculateBuildingSummary(
  ...
  // Smart fallback: เฉพาะ NANA ถ้าเดือนนั้นไม่มี record ของ floor1RentIncome
  // ระบบจะเติม default 23,000 ให้อัตโนมัติ (user สามารถ override โดยกดบันทึก/แก้ไข)
  ...
  // (อาคารอื่น เช่น FUNN1/FUNN2 จะไม่ใช้ค่านี้อยู่แล้ว เพราะ UI gate ด้วย isEligibleForSalary)
  const eligibleBuildingsForSalaryCheck = ['CT', 'YW', 'NANA']
  if (useLeaveForFdExtra && eligibleBuildingsForSalaryCheck.includes(building.code)) {
  ...
  // ดึงค่าใช้จ่ายแยกตามอาคาร จาก ExpenseHistory (ทุกอาคาร)
  ...
  return {
    ...
    // ค่าแรงวันหยุดชดเชย (เฉพาะ CT/YW/NANA)
    holidayCompensation: holidayCompensationPerBuilding,
